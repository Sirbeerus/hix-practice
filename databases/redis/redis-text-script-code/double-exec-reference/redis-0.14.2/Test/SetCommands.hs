...
tests = TList [TLabel "smembers" test_smembers,
               ...
               TLabel "variadic sadd" test_variadic_sadd,
               TLabel "variadic srem" test_variadic_srem]
...
test_variadic_sadd = TCase $ testRedis $
    do r <- ask
       liftIO $ do sadd_ r "set2" ["1", "2", "3"]
                   s2 <- smembers r "set2" >>= asSet
                   assertEqual "" (fromList [Just "1", Just "2", Just "3"]) s2

test_variadic_srem = TCase $ testRedis $
    do r <- ask
       addSet
       liftIO $ do res <- srem_ r "set" ["1", "3", "5"] >>= fromRInt
                   assertEqual "Two elements removed" 2 res
                   res <- smembers r "set" >>= asSet
                   assertEqual "" (fromList [Just "2"]) res

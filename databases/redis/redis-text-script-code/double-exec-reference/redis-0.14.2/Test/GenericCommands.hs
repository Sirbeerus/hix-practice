...
tests = TList [TLabel "from*" test_unwrap_reply,
               ...
               TLabel "variadic del" test_variadic_del]
...
test_variadic_del = TCase $ testRedis $
    do r <- ask
       liftIO $ do mSet r [("a", "a"), ("b", "b"), ("c", "c")]
                   del_ r ["a", "c"] >>= fromRInt >>= assertEqual "" 2
                   Just k <- keys r "*" >>= fromRMultiBulk
                   assertEqual "" [Just "b"] k

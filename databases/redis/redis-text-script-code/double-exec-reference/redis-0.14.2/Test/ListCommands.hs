...
tests = TList [TLabel "lrange" test_lrange,
               ...
               TLabel "variadic lpush and rpush" test_variadic_lpush_rpush]
...
test_variadic_lpush_rpush = TCase $ testRedis $
    do r <- ask
       liftIO $ do lpush_ r "list" ["1", "2"]
                   rpush_ r "list" ["3", "4"]
                   Just list <- lrange r "list" takeAll >>= fromRMultiBulk
                   assertEqual "" [Just "2", Just "1", Just "3", Just "4"] list

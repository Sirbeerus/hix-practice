...
tests = TList [TLabel "get and set" test_set_get,
               ...
               TLabel "incrByFloat" test_incrByFloat]
...
test_incrByFloat = TCase $ testRedis $
    do r <- ask
       liftIO $ do set r "a" (1 :: Int) >>= noError
                   incrByFloat r "a" 0.2 >>= fromRBulk >>= assertEqual "" (Just (1.0 + 0.2))
                   get r "a" >>= fromRBulk >>= assertEqual "" (Just (1.0 + 0.2 :: Double))
